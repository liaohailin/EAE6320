	...
	out float4 o_color : COLOR0 )
{
	...
	float3 color_sample2= tex2D(g_color_sampler2, i_uv).rgb;
	

	color_sample = (color_sample + color_sample2)*0.5;
	// The output color is the texture color
	// modified by the interpolated per-vertex color
	// and the per-material constant
	o_color = float4(color_sample * g_colorModifier * i_color_perVertex,
