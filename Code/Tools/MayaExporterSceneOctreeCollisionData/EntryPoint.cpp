...
namespace
{
	...
	const char* s_pluginName = "liao_hailin's EAE6320 Scene Octree Collision Format";
}
...
__declspec(dllexport) MStatus initializePlugin( MObject io_object )
{
	...
	{
		char* noIcon = "none";
		status = plugin.registerFileTranslator(s_pluginName, noIcon, eae6320::cMayaExporterSceneOctreeCollisionData::Create);
		if ( !status )
		{
