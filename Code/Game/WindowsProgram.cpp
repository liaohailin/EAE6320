...
HWND CreateMainWindowHandle( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState )
{
	...
	HWND mainWindow;
	{
		...
		const char* windowCaption = "[Hailin]'s EAE6320 Game";
		// The window's style
	...
	// Change the window's size based on the desired client area resolution
	{
		...
		const int desiredWidth = eae6320::UserSettings::GetWidth();
		const int desiredHeight = eae6320::UserSettings::GetHeight();
...
LRESULT CALLBACK OnMessageReceived( HWND i_window, UINT i_message, WPARAM i_wParam, LPARAM i_lParam )
{
	...
	if (i_message == WM_LBUTTONDOWN)
	{
		SetCapture(i_window);
		int mouse_x = (short)LOWORD(i_lParam);
		int mouse_y = (short)HIWORD(i_lParam);
		GetArcBall()->OnBegin(mouse_x, mouse_y);
	}
	if (i_message == WM_MOUSEMOVE)
	{
		int mouse_x = (short)LOWORD(i_lParam);
		int mouse_y = (short)HIWORD(i_lParam);
		GetArcBall()->OnMove(mouse_x, mouse_y);
	}
	if (i_message == WM_LBUTTONUP)
	{
		GetArcBall()->OnEnd();
		ReleaseCapture();
	}
